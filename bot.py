...
WORLDBUFF_CHANNEL_ID = 1281152286772695071
GUILD_INTERNAL_CHANNEL_ID = 1118795108968574987
POST_CHANNEL_ID = WORLDBUFF_CHANNEL_ID
...
def raid_announcement_signature(raid):
    ...
    player_pin = str(raid.get("playerPin") or "").strip().upper()
    return "|".join([raid_short, raid_date, raid_time, player_pin]).lower()


def raid_announcement_markers(raid):
    raid_name = str(raid.get("raidName") or normalize_raid_name(raid.get("raid") or raid.get("raidName") or "") or "Raid").strip()
    raid_date = format_raid_announcement_date(raid.get("raidDate"))
    raid_time = format_raid_announcement_time(raid.get("raidTime"))
...
async def raid_announcement_already_in_channel(channel, raid, limit=40):
    markers = raid_announcement_markers(raid)
    try:
        ...
        print(f"Raid-Ankuendigung Channel-Pruefung fehlgeschlagen: {e}")
    return False
...
def build_worldbuff_replacement_text(payload):
    ...
    charakter = str(payload.get("charakter") or payload.get("werfer") or payload.get("caster") or "").strip()
    note = str(payload.get("note") or payload.get("notiz") or "").strip()
    lichtloot_url = str(payload.get("url") or "https://lichtloot.de/").strip()

    lines = [
        f"⚠️ **Ersatz für {buff} gesucht**",
        ""
    ]
    details = []
    if datum:
        details.append(f"📅 **{datum}**")
    if uhrzeit:
        details.append(f"⏰ **{uhrzeit} Uhr**")
    if gilde:
        details.append(f"🏰 **{gilde}**")
    if details:
        lines.append(" · ".join(details))

    if charakter and charakter != "-" and not charakter.lower().startswith("ersatz"):
        lines.append(f"👤 Aktuell eingetragen: **{charakter}**")
    if note and note != charakter:
        lines.extend(["", f"📝 {note}"])

    lines.extend([
        "",
        "Wer den Termin übernehmen kann, trägt sich bitte direkt über LichtLoot ein:",
        f"➡️ {lichtloot_url}"
    ])
    return "\n".join(lines)[:1900]


async def post_worldbuff_replacement_request(payload):
    # Nur Ersatzgesuche gehen in beide Channels. Die normale Worldbuff-Uebersicht nutzt POST_CHANNEL_ID.
    target = str(payload.get("target") or "both").lower().strip()
    channel_ids = []
    if target in ("worldbuff", "both", "alle", "beide"):
        channel_ids.append(WORLDBUFF_CHANNEL_ID)
    if target in ("guild", "gilde", "intern", "gildenintern", "both", "alle", "beide"):
        channel_ids.append(GUILD_INTERNAL_CHANNEL_ID)

    channel_ids = list(dict.fromkeys(channel_ids))
    print(f"Worldbuff-Ersatzsuche: Ziel={target}, Channels={channel_ids}, Payload={payload}", flush=True)

    if not channel_ids:
        print("Worldbuff-Ersatzsuche: Keine Ziel-Channel gefunden.", flush=True)
        return
    ...
    for channel_id in channel_ids:
        try:
            ...
            posted += 1
            print(f"Worldbuff-Ersatzsuche gepostet in Channel {channel_id}.", flush=True)
        except discord.Forbidden as e:
            print(f"Worldbuff-Ersatzsuche: Kein Zugriff auf Channel {channel_id}: {e}", flush=True)
        except Exception as e:
            print(f"Worldbuff-Ersatzsuche: Fehler in Channel {channel_id}: {e}", flush=True)

    if posted == 0:
        raise RuntimeError("Worldbuff-Ersatzsuche konnte in keinen Ziel-Channel posten.")
...
async def handle_lichtloot_queue_item(item):
    ...
    if update_type == "worldbuff_replacement_request":
        await post_worldbuff_replacement_request(payload)
    ...
    while not client.is_closed():
        try:
            ...
            if result.get("success"):
                ...
                if items:
                    print(f"LichtLoot-Queue: {len(items)} offene Aufgabe(n) gefunden.", flush=True)
                for item in items:
                    try:
                        print(f"LichtLoot-Queue verarbeitet Typ={item.get('type')} Row={item.get('rowNumber')}", flush=True)
                        await handle_lichtloot_queue_item(item)
                    except Exception as item_error:
                        print("Fehler beim Verarbeiten eines LichtLoot-Queue-Eintrags:", item_error, flush=True)
            else:
                print("LichtLoot-Queue Antwort:", result, flush=True)
...
async def raid_announcement_loop():
    ...
    while not client.is_closed():
        try:
            ...
            posted_signatures = set(str(x) for x in state.get("posted_raid_signatures", []))
            ...
            for raid in raids:
                ...
                signature = raid_announcement_signature(raid)

                if raid_id not in posted_ids and signature not in posted_signatures:
                    if initialized:
            ...
            for raid in new_posts:
                ...
                try:
                    if await raid_announcement_already_in_channel(channel, raid):
                        print(f"Raid-Ankuendigung bereits im Channel vorhanden, ueberspringe: {raid.get('raidId')} in {channel_id}")
                        posted_ids.add(str(raid.get("raidId") or "").strip())
                        posted_signatures.add(raid_announcement_signature(raid))
                        continue
            ...
            state["posted_raid_signatures"] = sorted(posted_signatures)
...
@client.event
async def on_ready():
    print(f"Bot online als {client.user}", flush=True)
    print(f"Überwache Ticker-Channel: {TICKER_CHANNEL_ID}", flush=True)
    print(f"Worldbuff-Channel: {WORLDBUFF_CHANNEL_ID}", flush=True)
    print(f"Gildenintern-Channel: {GUILD_INTERNAL_CHANNEL_ID}", flush=True)
    print(f"Postet Übersicht in Channel: {POST_CHANNEL_ID}", flush=True)
    print(f"Hordenbuff-Channel: {HORDENBUFF_CHANNEL_ID}", flush=True)
    print(f"LichtLoot-Queue: {'aktiv' if LICHTBOT_QUEUE_TOKEN else 'deaktiviert - LICHTBOT_QUEUE_TOKEN fehlt'}", flush=True)
    print("Version 4.10 gestartet: Worldbuff-Ersatzsuche und Queue-Logs aktiv.", flush=True)
