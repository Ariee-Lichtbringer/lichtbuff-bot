...
def build_worldbuff_replacement_text(payload):
    ...
    lines = [
        f"⚠️ **Ersatz gesucht: {buff}**",
        ""
    ]
    if datum:
        lines.append(f"📅 Datum: **{datum}**")
    if uhrzeit:
        lines.append(f"⏰ Uhrzeit: **{uhrzeit} Uhr**")
    if gilde:
        lines.append(f"🏰 Gilde: **{gilde}**")
    if charakter and charakter != "-":
        lines.append(f"👤 Eingetragen: **{charakter}**")
    if note:
        lines.extend(["", f"📝 {note}"])
    lines.extend(["", "Bitte meldet euch, wenn ihr diesen Termin übernehmen könnt."])
    return "\n".join(lines)[:1900]
