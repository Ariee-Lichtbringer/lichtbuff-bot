...
def collect_message_text(message):
    """Sammelt Text aus normalem Inhalt und Embeds einer Discord-Nachricht."""
    parts = []
...
def sende_wurf_ans_sheet(buff, charakter, discord_name):
    ...
    try:
        ...
        raise RuntimeError(f"Apps Script konnte nicht erreicht werden: {e}")

    try:
        return json.loads(raw)
    except Exception:
        raise RuntimeError(f"Apps Script hat keine gültige JSON-Antwort geliefert: {raw[:1500]}")
...
async def on_message(message):
    ...
    if lower.startswith("!wurf "):
        ...
        try:
            ...
            fehlertext = str(e)
            print(f"Fehler bei !wurf: {fehlertext}")
            await message.channel.send(
                "⚠️ Beim Eintragen ist ein Fehler passiert. Genauere Fehlermeldung:\n"
                f"```{fehlertext[:1500]}```"
            )
        return
