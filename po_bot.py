...
class PoBotV2(discord.Client):
    def __init__(self, api: LichtLootApi) -> None:
        ...
        self.forever = ForeverWorker(self, lambda c: _emoji({"tank":"tank","heal":"heilung","dd":"melee","melee":"melee","ranged":"range"}[c], {"tank":"🛡️","heal":"✨","dd":"⚔️","melee":"⚔️","ranged":"🏹"}[c]) if c in {"tank","heal","dd","melee","ranged"} else str(_class_select_emoji(FOREVER_CLASSES.get(c,c)) or ""))
            ...
            try:
                result = await self.api.get(
                    ...
                    types=(
                        ...
                        "p0plus_points_notice,p0plus_resolution_notice,player_login_approval_notice,po_approval_notice,po_rejection_notice,po_release_granted_notice,po_release_received_notice,armor_request_notice,guild_bank_notice,guild_bank_request_notice,po_release_request_notice,raid_calendar,raid_signup_notice,raid_status_staff_notice"
                    ),
                    limit="20",
                    claimMode="manual-v1",
                )
        ...
        try:
            ...
            if clean(selected.get("entrySource")) == "po_post":
                ...
                await self.bot.api.review_p0_signup(
                    ...
                    self.raid_id,
                    signup_id=entry_id,
                    storage=clean(selected.get("storage")),
                    status=self.review_status,
...
def build_combined_embed(
    ...
    # Gildenleitung konfigurierte Bild (mit allgemeiner Vorlage als Fallback).
    if raid_signup_enabled:
        raid_key = _canonical_raid_key(raid)
