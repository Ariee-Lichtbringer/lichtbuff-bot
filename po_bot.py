...
def lichtloot_help_embed():
    embed = discord.Embed(
        ...
        description=(
            ...
            "Wähle unten ein Thema aus oder klicke auf **Eigene Frage stellen**. "
...
async def send_problem_to_ariee(interaction, problem, source="Lichtloot-Hilfe"):
    ...
    embed.set_footer(text="Bitte antworte dem Spieler direkt auf Discord.")
    try:
        await ariee.send(embed=embed)
        return True, ""
    except Exception as error:
...
class LichtlootQuestionModal(discord.ui.Modal, title="Eigene Lichtloot-Frage"):
    ...
    async def on_submit(self, interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                "Deine Frage passt noch zu keinem hinterlegten Hilfethema. Bitte wende dich an **Ariee** oder "
                "die Gildenleitung. Gib dabei deinen Charakternamen und – falls vorhanden – die genaue Fehlermeldung an."
            ),
            color=discord.Color.orange(),
        )
        embed.add_field(name="Deine Frage", value=question[:1024], inline=False)
        await interaction.response.defer(ephemeral=True)
        sent, error = await send_problem_to_ariee(interaction, question, "Nicht beantwortete freie Frage")
        if sent:
            embed.add_field(
